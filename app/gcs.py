...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe("telemetry/imu")
    client.subscribe("gcs/ui_state") 
    client.subscribe("drone/stream/yolo") 

def on_message(client, userdata, msg):
    global drone_yaw, target_yaw, nav_state, latest_frame_data
    payload = msg.payload.decode()
    
    if msg.topic == "cmd/target_angle":
        try: target_yaw = float(payload)
        except: pass
    elif msg.topic == "telemetry/imu":
        try:
            data = json.loads(payload)
            drone_yaw = data['yaw'] # NOW USING PRECISE ABSOLUTE YAW
        except: pass
    elif msg.topic == "gcs/ui_state":
        try: nav_state = json.loads(payload)
        except: pass
    elif msg.topic == "drone/stream/yolo":
...
def send_drop():
    mqtt_client.publish("cmd/payload_drop", "GO")
...
def update_ui():
    ...
    elif nav_state["status"] == "SEARCHING": 
        radar.coords(target_dot, -10, -10, -10, -10) 
        btn_drop.config(state=tk.DISABLED, bg="gray")
        
        # Check if actually moving or waiting
        if nav_state.get("dir_y") == "FORWARD":
            instr_label.config(text="SEARCHING: MOVING FORWARD", fg="orange")
            nav_buttons["FORWARD"].config(bg="#FF8800")
        else:
            instr_label.config(text="SEARCHING: AWAITING FRAME", fg="yellow")

    elif nav_state["status"] == "ADJUSTING":
        dir_x = nav_state.get("dir_x", "NONE")
        instr_label.config(text=f"ALIGNING: TURN {dir_x}", fg="cyan")
        radar.coords(target_dot, -10, -10, -10, -10)
        btn_drop.config(state=tk.DISABLED, bg="gray")
        if dir_x in nav_buttons:
            nav_buttons[dir_x].config(bg="#FF8800")
